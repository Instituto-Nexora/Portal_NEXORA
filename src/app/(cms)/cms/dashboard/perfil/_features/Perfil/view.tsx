  ...
  return (
    <div className={cn("min-w-0 space-y-6 overflow-hidden")}>
      <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between")}>
        ...
        <div className={cn("rounded-full border bg-muted px-3 py-1 text-xs text-muted-foreground")}>
          Perfil CMS · {initialData.role}
        </div>
      </div>

      <div className={cn("grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_22rem]")}>
            ...
            <CardContent>
              ...
              >
                ...
                {statusPerfil?.formId === "perfil" && (
                  <Alert variant={statusPerfil.success ? "default" : "destructive"}>
            ...
            <CardContent>
              ...
              >
                ...
                {statusSenha?.formId === "senha" && (
                  <Alert variant={statusSenha.success ? "default" : "destructive"}>
            ...
            <CardContent>
              <form action={avatarFormAction} className={cn("space-y-5")}>
                ...
                <div className={cn("space-y-1 text-center")}>
                  <p className={cn("text-sm font-medium")}>{initialData.fullName}</p>
                  <p className={cn("text-xs text-muted-foreground")}>{initialData.email}</p>
                </div>

                <CooldownHint value={initialData.nextAvatarChangeAt} />

                {statusAvatar?.formId === "avatar" && (
                  <Alert variant={statusAvatar.success ? "default" : "destructive"}>
