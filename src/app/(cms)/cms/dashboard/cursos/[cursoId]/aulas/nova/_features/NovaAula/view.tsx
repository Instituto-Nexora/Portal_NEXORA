...
export function NovaAulaView({ cursoId }: Props) {
  const { form, onSubmit, isPending, state } = useNovaAulaViewModel(cursoId)
  const { register, handleSubmit, watch, setValue, formState: { errors } } = form

  return (
    <div className={cn("p-6 max-w-2xl mx-auto space-y-6")}>
      <div className={cn("flex items-center gap-3")}>
        <Link href={`/cms/dashboard/cursos/${cursoId}`}>
          <Button variant="ghost" size="sm" aria-label="Voltar para lista de aulas">
            <ArrowLeft className={cn("w-4 h-4")} aria-hidden="true" />
          </Button>
        </Link>
        <div>
          <h1 className={cn("text-2xl font-bold text-foreground")}>Nova Aula</h1>
          <p className={cn("text-sm text-muted-foreground")}>Preencha os dados da aula.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className={cn("space-y-5")}>
        ...
        <div className={cn("flex items-center gap-3 py-1")}>
          <input
            ...
            aria-label="Publicar aula imediatamente"
          />
