...
export function NovoCursoView() {
  const { form, onSubmit, isPending, state } = useNovoCursoViewModel()
  ...
  return (
    <div className={cn("p-6 max-w-2xl mx-auto space-y-6")}>
      <div className={cn("flex items-center gap-3")}>
        ...
        <div>
          <h1 className={cn("text-2xl font-bold text-foreground")}>Novo Curso</h1>
          <p className={cn("text-sm text-muted-foreground")}>Preencha os dados do curso.</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className={cn("space-y-5")}>
        ...
        <div className={cn("flex items-center gap-3 py-1")}>
          <input
            ...
            aria-label="Publicar curso imediatamente"
          />
