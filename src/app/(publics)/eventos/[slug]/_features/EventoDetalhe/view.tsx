    ...
    <main>
      <div className={cn("max-w-6xl mx-auto px-6 pt-10")}>
        <Link
          href="/eventos"
          className={cn(
            "text-sm text-teal-700 hover:underline mb-6 inline-block",
          )}
        >
          ← Voltar para Eventos
        </Link>
      ...
      <div className={cn("max-w-6xl mx-auto px-6 pb-12")}>
        ...
        {evento.long_description && (
          <div
            className={cn(
              "mb-8 text-slate-700 leading-relaxed whitespace-pre-line",
            )}
          >
            {evento.long_description}
          </div>
      ...
      {outrosEventos.length > 0 && (
        <div className={cn("max-w-6xl mx-auto px-6 pb-16")}>
          <h2 className={cn("text-xl font-bold text-slate-900 mb-6")}>
            Outros eventos
          </h2>
          <ul
            className={cn(
            ...
            {outrosEventos.map((outro) => (
              <li key={outro.id}>
                ...
                >
                  <Card
                    className={cn(
                      "h-full overflow-hidden border-t-4 border-t-teal-600 pt-0 hover:shadow-md hover:-translate-y-1 transition-all duration-300",
                    )}
                  >
                    <div
                      className={cn(
                    ...
                    >
                      <Image
                        src={
                          outro.thumbnail_url ?? "/images/event-placeholder.jpg"
                        }
                        alt={outro.title}
                        fill
                        className={cn(
                    ...
                    <CardHeader className={cn("pb-2")}>
                      ...
                      <CardTitle
                        className={cn("text-sm leading-snug text-slate-900")}
                      >
                        {outro.title}
                      </CardTitle>
                    </CardHeader>
                    <CardContent
                      className={cn("text-xs text-slate-500 leading-relaxed")}
                    >
                      {outro.description}
                    </CardContent>
                  </Card>
