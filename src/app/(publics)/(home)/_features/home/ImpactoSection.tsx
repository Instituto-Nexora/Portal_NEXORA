  ...
  return (
    ...
    >
      <div className={cn("max-w-4xl mx-auto text-center")}>
        <h2
          id="impacto-title"
          className={cn("text-3xl font-bold text-slate-900 mb-2")}
        >
        ...
        <div
          className={cn(
            "flex flex-wrap justify-center divide-x divide-teal-200",
          )}
        >
          {items.map((item) => (
            <div
              key={item.id}
              className={cn("flex flex-col items-center gap-2 px-12 py-4")}
            >
              <span className={cn("text-6xl font-black text-teal-700")}>
                {item.value}
              </span>
              <span className={cn("text-sm text-slate-600 font-medium")}>
                {item.label}
              </span>
            </div>
          ))}
