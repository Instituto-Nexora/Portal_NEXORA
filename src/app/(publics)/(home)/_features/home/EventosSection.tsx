...
export function EventosSection({
  ...
  return (
    <section
      ...
      id="eventos"
      aria-labelledby="eventos-home-title"
    >
      <div className={cn("max-w-5xl mx-auto")}>
        <div
          className={cn("flex items-end justify-between mb-12 gap-4 flex-wrap")}
        >
          <div>
            <span
              className={cn(
                "text-teal-600 text-xs font-semibold tracking-widest uppercase block mb-2",
              )}
            >
              Ao vivo & Gravados
            </span>
            <h2
              id="eventos-home-title"
              className={cn("text-3xl font-bold text-slate-900")}
            >
              Eventos
            </h2>
          </div>
          <Link
            href="/eventos"
            className={cn(
              "text-sm font-semibold text-teal-700 hover:text-teal-600 underline-offset-4 hover:underline transition-colors shrink-0",
            )}
          >
        ...
        {proximosEventos.length > 0 && (
          <div className={cn("mb-14")}>
            <h3
              className={cn(
                "text-lg font-semibold text-slate-900 mb-6 flex items-center gap-2",
              )}
            >
              <span
                className={cn("size-2 rounded-full bg-amber-500 animate-pulse")}
                aria-hidden="true"
              />
              Próximos Eventos
            </h3>
        ...
        {eventosGravados.length > 0 && (
          <div>
            ...
            >
              <PlayCircle
                className={cn("size-4 text-teal-600")}
                aria-hidden="true"
              />
              Eventos Gravados
            </h3>
                ...
                <li key={evento.id} className={cn("flex")}>
                  ...
                  >
                    <a
                      ...
                      className={cn(
                        "relative block overflow-hidden aspect-video",
                      )}
                    >
                      <Image
                        src={evento.imageUrl}
